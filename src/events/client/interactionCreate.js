...
module.exports = async (client, interaction) => {
    ...
    if (interaction.isCommand() || interaction.isUserContextMenuCommand()) {
        try {
            const banData = await banSchema.findOne({ where: { User: interaction.user.id } });
            if (banData) {
            ...
            if (interaction.options._subcommand !== null && interaction.options.getSubcommand() == "help" && interaction.commandName !== "help") {
    ...
    if (interaction.isButton() && interaction.customId == "Bot_verify") {
        const data = await verify.findOne({ 
            where: { Guild: interaction.guild.id, Channel: interaction.channel.id } 
        });
        if (data) {
            // Temporarily disabled captcha due to canvas compilation issues
            client.errNormal({
                error: "Captcha verification is temporarily disabled. Please contact an admin for manual verification.",
                type: 'ephemeral'
            }, interaction);
    ...
    if (interaction.isButton()) {
        ...
        if (buttonID[0] == "reaction_button") {
            try {
                const data = await reactionSchema.findOne({ where: { Message: interaction.message.id } });
                if (!data) return;
    ...
    if (interaction.isStringSelectMenu()) {
        if (interaction.customId == "reaction_select") {
            try {
                ...
                console.error('Reaction role select error:', error);
            }
        }
    }
    // Tickets
